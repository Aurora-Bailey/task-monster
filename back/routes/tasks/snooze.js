...
async function snoozeTaskRoute(app) {
	app.post(
		'/tasks/:taskId/snooze',
		{
			schema: {
		...
		async (request, reply) => {
			...
			if (!task.activeToday || !task.alarmEnabled || !task.snoozeMinutes) {
				return reply.code(409).send({
					message: 'Task cannot be snoozed.'
				});
			}

			const snoozedAt = new Date();
			const alarmDueAt = new Date(snoozedAt.getTime() + task.snoozeMinutes * 60 * 1000);
			...
			if (!result) {
				return reply.code(409).send({
					message: 'Task could not be snoozed.'
				});
			}
...
module.exports = snoozeTaskRoute;
