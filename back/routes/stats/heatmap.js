					...
					items: {
						...
						properties: {
							...
							sessions: {
								...
								items: {
									type: 'object',
									required: ['id', 'taskId', 'name', 'color', 'colorKey', 'startedAt', 'endedAt'],
									properties: {
										id: { type: 'string' },
										taskId: { type: 'string' },
										name: { type: 'string' },
...
async function heatmapStatsRoute(app) {
	app.get(
		...
		async (request, reply) => {
			...
			if (request.query.startDay !== undefined && !isValidDayString(request.query.startDay)) {
				return reply.code(400).send({
					message: 'Invalid start day.'
				});
			}

			const startDay = request.query.startDay || getCurrentLocalDay(timezoneOffsetMinutes);
			...
			const tasksById = new Map(
				tasks.map((task) => [
					task._id.toString(),
					{
						...
						name: typeof task.name === 'string' && task.name ? task.name : 'Unknown task',
						color: typeof task.colorHex === 'string' && task.colorHex ? task.colorHex : '#6f7d8b',
						colorKey: typeof task.colorKey === 'string' && task.colorKey ? task.colorKey : 'unknown'
					}
				])
			);
